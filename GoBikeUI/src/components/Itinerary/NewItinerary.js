...
class NewItinerary extends React.Component {
  ...
  getChatStatus = () => {
    const targetGetUrl =
      ...
      fetch(targetGetUrl, {
        ...
        .then((data) => {
          console.log(data);
          this.setState({ sharedWithUser: data });
  })
}
  ...
  shareChat = () => {
    ...
    var targetUrl = config.API_URL + "/userchat/" + window.localStorage.getItem("username") + "/true";
  ...
  .then((response) => {
    if (response.status == "200") {
      console.log("chat is started with other user too")
    }
  })}
    ...
    fetch(targetUrl, requestOptions)
      .then((response) => {
        if (response.status == "200") {
          ...
          window.localStorage.setItem("myusers", this.state.user[0]);
          console.log(window.localStorage.getItem("myusers"));
          this.setState({
            open: false,
          });
        }
    ...
    if (itineraries.length === 0) {
      ...
      return (
        <div>
          ...
          {this.state.itineraries.map((itinerary) => (
            <div>
              ...
              <Accordion>
                ...
                <AccordionDetails>
                  <Typography>
                    This will include information about your itinerary, as well
                    as a description if applicable.
                  </Typography>
                </AccordionDetails>
                <AccordionDetails>
                  <Typography>
          ...
          {/* if the response is true from backend then n only then this flag will set to true else false */}
          {/* {this.state.sharedWithUser === true ? (<ChatApp/>) : null} 
          {console.log("sjaredddd" ,window.localStorage.getItem('shared'))} */}
          {/*  sharedwithUser is global flag which sets true when chat is shared with user for entire session */}
